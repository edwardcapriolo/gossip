...
import io.teknek.gossip.event.data.DataEventManager;
...
public class GossipCore implements GossipCoreConstants {
  ...
  public static final Logger LOGGER = LoggerFactory.getLogger(GossipCore.class);
  ...
  public void mergeLists(RemoteMember senderMember, List<Member> remoteList) {
    ...
    for (LocalMember i : gossipManager.getDeadMembers()) {
      if (i.getId().equals(senderMember.getId())) {
        LOGGER.debug("{} contacted by dead member {}", gossipManager.getMyself(), senderMember.getUri());
